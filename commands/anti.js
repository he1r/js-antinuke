...
.setThumbnail(message.author.displayAvatarURL({dynamic: true}))
  ...
  let check = bot.settings.get(message.guild.id, "antiping")
  if(check === false) {
    let embed16 = new MessageEmbed()
    .setColor(colors.green)
    .setAuthor(`Anti Ping Enabled`, message.guild.iconURL({dynamic: true}))
  .setDescription(`
<:DE_OnGreen:805196654595932180> **Anti Everyone/Here Ping**`)
  .setTimestamp()
  .setThumbnail(message.author.displayAvatarURL({dynamic: true}))
  	message.channel.send(embed16)
  	bot.settings.set(message.guild.id, true, "antiping")
  } else if (check === true) {
    let embed17 = new MessageEmbed()
    .setColor(colors.red)
    .setAuthor(`Anti Ping Disabled`, message.guild.iconURL({dynamic: true}))
  .setDescription(`
<:DE_Off:805196659260391445> **Anti Everyone/Here Ping**`)
.setTimestamp()
.setThumbnail(message.author.displayAvatarURL({dynamic: true}))
  	  	message.channel.send(embed17)
  	bot.settings.set(message.guild.id, false, "antiping")
  }
  }
}
module.exports.help = {
    name: "anti",
    aliases: ['antinuke', 'anti-nuke', 'toggle-anti'],
}
