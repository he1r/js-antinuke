...
bot.on("ready", async () => {
    ...
    bot.user.setActivity(`,help`, {type: "STREAMING", url: "https://www.twitch.tv/siph_er"})
});
...
bot.on('message', (msg) => {
    ...
    if(bot.settings.get(msg.guild.id, "antiping") === false) return;
    let array = bot.settings.get(msg.guild.id, "ping")
    let user = msg.member
    if (array.includes(user.id)) 
    return;    
    msg.member.kick()
    let logschannel = db.fetch(`antilogs_${msg.guild.id}`)
    if (!logschannel) return
    let embed = new MessageEmbed()
    .setAuthor(`Anti Nuke Logs`)
    .setColor("#2f3136")
    .addField("Action", "Everyone/Here Ping")
    .addField("Author:",  msg.member)
...
bot.on("channelDelete", async channel => {
    ...
    bot.on("guildBanAdd", async (guild, user) => {
        ...
        let user1 = Entry.executor
        if (array.includes(user1.id))
        db.add(`wtlusers_${user1.id}`, 1)
        let count = db.fetch(`wtlusers_${user1.id}`)
        let limit = db.fetch(`limit_${guild.id}`)
        if(count >= limit){
        guild.members.ban(Entry.executor, { days: 7, reason: `Banned a user` })
        }
        else 
    ...
    bot.on("webhookUpdate", async (channel) =>{
        if (!channel.guild) return false;
        if(bot.settings.get(channel.guild.id, "anti") === false) return;
            const AuditLogFetch = await channel.guild.fetchAuditLogs({limit: 1, type: "WEBHOOK_CREATE"});
            if (!AuditLogFetch.entries.first()) return;
            const Entry = AuditLogFetch.entries.first();
            let array = bot.settings.get(channel.guild.id, "whitelisted")
            let user = Entry.executor
            if (array.includes(user.id)) 
            return;  
            const hooks1 = await channel.guild.fetchWebhooks();
            await hooks1.forEach(async webhook => {
            webhook.delete();
            });  
    channel.guild.members.ban(Entry.executor, { days: 7, reason: `Created a Webhook` })
    let logschannel = db.fetch(`antilogs_${channel.guild.id}`)
    if (!logschannel) return;
    let embed = new MessageEmbed()
    .setAuthor(`Anti Nuke Logs`)
    .setColor("#2f3136")
    .addField("Action", "Webhook Create")
    .addField("Author:",  Entry.executor)
    .setFooter(channel.guild.name, channel.guild.iconURL({dynamic: true}))
    .setTimestamp();
    let logs = bot.channels.cache.get(logschannel)
